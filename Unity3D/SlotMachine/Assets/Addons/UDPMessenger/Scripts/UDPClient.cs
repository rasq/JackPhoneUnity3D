...
public class UDPClient : MonoBehaviour {		
	...
	void Update () {
        ...
        if (notifyObjects.Length > 0 && notifyObjects[0] == null) {
            Debug.Log("No object to notify.");
            if (GameObject.FindWithTag(objectTag)) {   //If we an object in the scene with the tag Player
                notifyObjects[0] = GameObject.FindWithTag(objectTag);    //Store that object as our users information object
            }
        }
